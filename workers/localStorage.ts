...
export const saveOpenNotification = async (payload: any) => {
  try {
    const response = await AsyncStorage.setItem(
      '@opennotify',
      JSON.stringify(payload),
...
export const getOpenNotification = async () => {
  try {
    const response = await AsyncStorage.getItem('@opennotify');
    let raw_data = JSON.parse(response);
    if (raw_data) {
      return raw_data;
    } else {
...
export const removeOpenNotification = async () => {
  try {
    const response = await AsyncStorage.removeItem('@opennotify');
    return true;
  } catch {
    return false;
  }
